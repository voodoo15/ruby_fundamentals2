...
while to_exit == false
    ...
    case choice
      ...
      when "w"
        puts "Which item do you want to see:  "
        item = gets.chomp

        if item.to_i <= grocery_items.count
          puts "The item in position #{item} is:  #{grocery_items[item.to_i - 1]}"
        else
          puts "The position is not avaiable in the list."
        end
